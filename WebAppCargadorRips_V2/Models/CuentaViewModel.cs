...
namespace WebAppCargadorRips_V2.Models
{
    ...
    {
        [Required]
        [RegularExpression(@"^(11001.[0-9]{6})*$", ErrorMessage = "Para {0} debe empezar con 11001, solo valores numéricos, sin espacios y 12 caracteres")]
    ...
    public class RegisterViewModel
    {
        ...
        [StringLength(100, ErrorMessage = "El número de caracteres de {0} debe ser mínimo {2}.", MinimumLength = 8)]
        [DataType(DataType.Password)]
        [RegularExpression(@"^(?=.*[A-Z])(?=.*\d)(?=.*[a-z])(?=.*\W){8}.*$", ErrorMessage = "La contraseña debe contener: mínimo 8 caracteres una letra mayúscula, una letra minúsculas, un número y caracteres especiales (! @ # $% ^ & *)")]
        [Display(Name = "Contraseña (*)")]
    ...
    public class RecuperarContraseniaViewModel
    {
        ...
        [RegularExpression(@"^(11001.[0-9]{6})*$", ErrorMessage = "Para {0} debe empezar con 11001, solo valores numericos, sin espacios y 12 caracteres")]
        [Display(Name = "Código Prestador (*)")]
