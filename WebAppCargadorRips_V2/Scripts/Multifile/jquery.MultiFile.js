						...
						$.each(files, function (i, file) {
							...
							$(MultiFile.wrapper).find('input[type=file]').not(e).each(function(){
								...
								$.each(FILE_LIST(this), function (i, file) {
									if(file.name){
										//console .log(file.name.substring(2, 0));
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			...
			toomuch: 'Los archivos seleccionados excedene el maximo de tamaño permitido ($size)',
			toomany: 'Muchos archivos seleccionados (max: $max)',
			toobig: '$file es muy grande (max $size)'
		},
